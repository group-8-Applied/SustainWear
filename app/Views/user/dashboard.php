...
<head>
	...
	<meta name="description" content="View a quick summary of your account" />
			...
			<section class="mb-6">
				<div class="flex flex-col sm:flex-row gap-4">
					<div class="flex-1 min-w-full sm:min-w-[200px] bg-white rounded-xl shadow-md p-6 sm:p-8">
						...
						<p class="text-xl sm:text-2xl font-extrabold mt-1"><?= count($donations) ?></p>
					</div>
					<div class="flex-1 min-w-full sm:min-w-[200px] bg-white rounded-xl shadow-md p-6 sm:p-8">
						<p class="font-bold text-lg">Pending</p>
						<p class="text-xl sm:text-2xl font-extrabold mt-1"><?= count(array_filter($donations, function($d) { return $d["status"] === "pending"; })) ?></p>
					</div>
					<div class="flex-1 min-w-full sm:min-w-[200px] bg-white rounded-xl shadow-md p-6 sm:p-8">
						<p class="font-bold text-lg">Approved</p>
						<p class="text-xl sm:text-2xl font-extrabold mt-1"><?= count(array_filter($donations, function($d) { return $d["status"] === "approved"; })) ?></p>
					</div>
				</div>
			</section>
		</main>
